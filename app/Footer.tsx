...
const COLUMNS = [
  {
    ...
    links: [
      ...
      { label: "Explorer", href: `${API_URL}/viz` },
    ],
  },
  {
    heading: "For agents",
    links: [
      ...
      { label: "OpenAPI spec", href: `${API_URL}/openapi.json` },
  ...
  },
  {
    heading: "Account",
    links: [
      { label: "Sign up", href: `${API_URL}/signup` },
      { label: "Login", href: `${API_URL}/login` },
      { label: "Dashboard", href: `${API_URL}/dashboard` },
    ],
  },
];
    ...
    <footer className="site-footer">
      <div className="site-footer-inner">
        <div className="site-footer-top">
          <div className="site-footer-brand">
            <a href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
              {LOGO(26)}
              <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>PMAxis</span>
            </a>
            <p className="site-footer-tagline">
              Prediction market data for applications and AI agents — one REST API, one WebSocket stream, one MCP server.
            </p>
          </div>
          <div className="site-footer-cols">
            {COLUMNS.map(col => (
              <div key={col.heading} className="site-footer-col">
                ...
                {col.links.map(l => (
                  <a key={l.label} href={l.href} className="site-footer-link">{l.label}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="site-footer-bottom">
          <span>© 2026 PMAxis. All rights reserved.</span>
          <span className="site-footer-agent-note">Built to be readable by humans and AI agents alike.</span>
        </div>
      </div>
    </footer>
  );
